...
namespace MFST {
	...
	bool Mfst::start(const Log::LOG& log)
	{
		...
		case LENTA_END:
		{
			MFST_TRACE4(log,"------>LENTA_END")
				std::cout << "------------------------------------------------------------------------------------------   ------" << std::endl;
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: Number of lines %d, Syntax analyze was done succesfully!", 0, lenta_size);
			std::cout << std::setw(4) << std::left << 0 << "Number of lines " << lex.table[lex.size - 1].sn << ", Syntax analyze was done succesfully!" << std::endl;
			rc = true;
			break;
		}
	...
	bool Mfst::savededucation(const Log::LOG& log)
	{
		MfstState state;
		GRB::Rule rule;
